<?php
  ...
  $conn = mysqli_connect(hostname: "localhost", username: "root", password: "", database: "form");

  if ($conn->connect_error){
    die('Conection Failed: ' .$conn->connect_error);
  }
  else{
    $stmt = $conn->prepare(query: "insert into login_tbl(first_name, email, username, password) values(?, ?, ?, ?)");
    $stmt -> bind_param("ssss", $first_name, $email, $username, $password);
    $stmt->execute();
